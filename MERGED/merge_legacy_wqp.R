...
legacy <- read.csv("prep_merge_legacy_2017-02-23.csv", stringsAsFactors = FALSE)
#wqp <- read.csv("prep_merge_wqp.csv", stringsAsFactors = FALSE)
wqp <- read.csv("prep_merge_wqp_2017-02-23.csv", stringsAsFactors = FALSE)
#==============================================================================
...
merged <- merged[order(merged$AGENCY, merged$SITE,
                       merged$ICPRB_NAME, #merged$PARAMETER,
                       merged$DATE,  
                       merged$Replicate.Number, merged$DEPTH), ]
#==============================================================================
merged$DATE <- as.Date(merged$DATE)
merged$MONTH <- format(merged$DATE, "%m")
merged$YEAR <- format(merged$DATE, "%Y")
merged <- merged[merged$YEAR >= 1972, ]
#==============================================================================
...
agg_my_data <- function(merged, my.id, gages){
  agg.count <- aggregate(AGENCY ~ my.id +
                           #SITE +
                           #ICP_ID +
                           ICPRB_NAME + #PARAMETER + 
                           YEAR, data = merged, length)
  ...
  #merged.2 <- merge(merged, agg.count2[, 1:2], by = c("SITE", "PARAMETER"), all.y = TRUE)
  merged.2 <- merge(merged, agg.count2[, 1:2], by = c("SITE", "ICPRB_NAME"), all.y = TRUE)
  table(merged.2[merged.2$SITE %in% "USGS-01578310", "ICPRB_NAME"])
  # Re-order the columns.
  merged.2 <- merged.2[, names(merged.2)]
  
  merged.3 <- merge(merged.2, sites, by = c("SITE", "SITE_NAME", "AGENCY", "LATITUDE", "LONGITUDE"), all = TRUE)
  final.df <- merge(merged.3, gages, by = c("ICP_ID", "SITE", "AGENCY"), all = TRUE)
  return(final.df)
}

agg_my_data <- function(merged, my.id, gages, sites){
...
final.site <- agg_my_data(merged, "SITE", gages, sites)
final.icp <- agg_my_data(merged, "ICP_ID", gages, sites)
#==============================================================================
setwd("H:/Projects/EPA3Trends/Data/Data_May2017")
write.csv(final.site, "wq_data_5_1_2017.csv", row.names = FALSE)
...
for (i in seq_along(unique(final.site$SITE))) {
  print(paste(i, "/", length(unique(final.site$SITE)), sep = ""))
  sub.final <- final.site[final.site$SITE %in% unique(final.site$SITE)[i], ]
  sub.final <- sub.final[, !names(sub.final) %in% "EVENT_ID"]
  dbWriteTable(con, unique(final.site$SITE)[i], sub.final, overwrite = TRUE , row.names = FALSE)
  #for (j in 1:10) print(j)
}
